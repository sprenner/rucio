...
create_and_move_dataset = function(scope, name, dids) {
    ...
    if(storage.isSet('moving_datasets') && found_dataset_list.length > 0) {     
            console.log('Dataset already exists, getting rules...');
            r.did_get_rules({
                'scope': scope,
                ...
                success: function(data) {
                    console.log(data);
                    if(data.length == 0) {
                        // Dataset exists but rule doesnt, so just create the rule again
                        ...
                        r.get_replica_lock_for_rule_id({
                            ...
                            success: function(suc) {
                                console.log(suc);
                               console.log(dids);
                                // TODO: not only compare length but also if same files; suc vs dids
                                if(suc.length != dids.length) { // if more/less locks than necessary exist, reset storage and restart process with new dataset name
