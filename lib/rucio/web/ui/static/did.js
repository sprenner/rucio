...
function extractLogfile(link, filename) {
    ...
    var jqXHR = $.ajax({
	    ...
	    var w = window.open();
	    //bd.replace(/(?:\r\n|\r|\n)/g, '<br />')
	    $(w.document.body).html('<h1>Content of ' + filename  + '</h1><textarea style="width:100%;height:40vw;resize:none;">' + res + '</textarea>');    
	},
	error: function(err) {
		$('#infomodal').html(tempHtml);
...
function runPyScript(input){
	...
	success: function(res) {
	    ...
	    $.each(res.split("\n"), function(index, value) {
	        if(index == 0) {
		    modalContent += value  + "</br>";
		} else {
		    modalContent += "<a href=\"#\" onclick=\"extractLogfile('" + input  + "', '" + value + "');return false;\">" + value  + "</a></br>";
		}
            });
