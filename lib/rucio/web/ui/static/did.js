            ...
            r.list_replicas({
                ...
                success: function(ret_dav_replicas) {
                    ...
                    if (typeof temp === "undefined") {
	                    //create object which contains all dav rse links and can be queried by replica name
	                    temp = {};
                                ...
                                try{
                                    if (browser_enabled_rses.indexOf(rse) != -1 && !blacklisted){ 
				                        ...
				                        if(link.includes(".log")){		
                                            str_rses += "<a href=\"#\" onclick=\"loadLogfile('" + temp[replica['name']][rse][0].replace("davs", "https")  +"');return false;\"><i title=\"Click to preview this file from " + rse  + ".\" class=\"fi-eye size-18\"></i></a> "; 
				                        }
                       
                                        
                                        str_rses += "<a href=\"#\" onclick=\"download_file('" + temp[replica['name']][rse][0].replace("davs", "https")  +"', '" + replica['scope']  + "', '" + replica['name'] + "', '" + rse + "', '" + replica['bytes']  + "', '" + scope  + "', '" + name + "');return false;\"><i title=\"Click to download this file from " + rse  + ".\" class=\"fi-download size-18\"></i></a> ";
                                        //str_rses += "<a href=\"" + temp[replica['name']][rse][0].replace("davs", "https") + "\" target=\"blank\">  <i title=\"Click to download this file from " + rse  + ".\" class=\"fi-download size-18\"></i></a>";
    		                        } else {                        
                                        //console.log(replica['scope'] + " " + replica['name'])                                                  
                                        str_rses += rse + " <a href=\"#\" onclick=\"move_did('" + replica['scope'] + "','" + replica['name'] + "',false,1);return false;\" style=\"color: rgb(0,255,0)\"><i class=\"fi-download size-18\"></i></a>";
			                        }
		                        } catch (err) {
                                    console.log("Caught Error for RSE " + rse + " : " + err);
                                    str_rses += rse;
                                }
...
move_did = function(scope, name, is_dataset, no_of_files) {
    console.log('moving did...');
    // TODO: was wenn auf der scratchdisk nicht genug platz ist
    var options = {
        dids:[{'scope': scope, 'name': name}], 
        account:r.account,
        copies:1, 
        lifetime:172800/*was 86400*/, 
        asynchronous:true, 
        notify:'Y', 
        rse_expression:scratch_rses[get_rse_index(scope + ':' + name, scratch_rses.length)], 
        success: function(info) {
            console.log(info);
            console.log('moving did successful!');
            // TODO: check if this works!
            if(is_dataset) {
                alert(no_of_files + ' file(s) in this dataset have to be moved to WebDAV enabled RSEs first, please come back later.');
            }
        }, 
        error: function(jqXHR, textStatus, errorThrown) {
            console.log(jqXHR);
...
extractLogfile = function(link, filename) {
    console.log("calling extract with: " + link + " " + filename)
    ...
    var jqXHR = $.ajax({
	    ...
	    var str = '';
	    console.log(parseInt(res.size) >= 20971520);
...
loadLogfile = function(input){
    ...
    var jqXHR = $.ajax({
	    ...
	    dataType: "json", // achtung, error wenn kein json zurückkommt bzw es nicht zu json geparsed werden kann!!
	    success: function(res) {
            console.log(res);
...
load_dataset_replicas = function(scope, name) {
    ...
    r.list_dataset_replicas({
	    ...
	    $.each(replicas, function(index, replica) {
            ...
            dt = $('#dt_dataset_replicas').DataTable( {
                data: data,
                bAutoWidth: false,
            ...
            /*var table = $('#dt_dataset_replicas').DataTable();
            $('#dt_dataset_replicas tbody').on('click', 'tr', function () {
                var data = table.row( this ).data();
                alert( 'You clicked on '+data.rse+'\'s row' );
	    } );
            table.draw();*/
...
get_metalink = function(scope, name) {
                    ...
                    success: function(ret) {
                        ...
                        $.each(dataset_files, function(index, item) {
                            if(meta_files.indexOf(item['scope'] + ':' + item['name']) == -1) {
                                ...
                                move_amount++; //TODO: erzsetzen durch files_to_move.length
                            }
                        });
                        ...
                        if(move_amount == 0) {
                            ...
                            //window.open('data:application/octet-stream,' + encodeURIComponent(ret_meta.replace(new RegExp('davs', 'g'), 'https')), "_self");    
                        ...
                        // TODO: anders resetten: entweder wenn not moved oder in allen zweigen von create_and_move_dataset
                        $('#meta_button').html('<a href="#" onclick="get_metalink(\'' + scope + '\',\'' + name + '\');return false;"><i title="Click to download metalink for this dataset." class="fi-download size-24"></i></a>');   
                    },
                    error: function(jqXHR, textStatus, errorThrown) {
    ...
    } else {alert('Dataset contains more than 500 files, no metalink created.');} 
};
...
create_and_move_dataset = function(scope, name, dids) {
    ...
    if(storage.isSet('moving_datasets')) {
        ...
        console.log(found_dataset_list);
    }
    ...
    if(storage.isSet('moving_datasets') && found_dataset_list.length > 0) {     
            ...
            r.did_get_rules({
                ...
                success: function(data) {
                    ...
                    if(data.length == 0) {
                        // Dataset exists but (single )rule doesnt, so just create the rule again
                        // TODO: check if correct files are still attached, suc vs dids
                        ...
                        r.did_get_files({
                            ...
                            success: function(suc) {
                                console.log(suc);
                                console.log(dids);
                                // check if all files good
                                $.each(dids, function(index, did){
                                    if(!isDidInArray(suc, did.scope, did.name)){
                                        //TODO: WICHTIG Files detachen und richtig attachen! sonst macht er hier nix mehr
                        ...
                        r.get_replica_lock_for_rule_id({
                            ...
                            success: function(suc) {
                                ...
                                console.log(dids); 
                                $.each(dids, function(index, did) {
                                    if(!isDidInArray(suc, did.scope, did.name)){ //not all necessary files in rule, break loop and create new rule
                                        console.log('Replicating and necessary amount of rules not matching, restarting...');
                                        // remove from storage list
                                        var new_dataset_list = $.grep(storage.get('moving_datasets'), function(obj){return obj.id != scope + ':' + name;});
                                        storage.set('moving_datasets', new_dataset_list);
                                        create_and_move_dataset(scope, name, dids);       
                                        return false; // break out from each loop
                                    }
        ...
        // TODO: storage irgendwann expiren lassen? Sonst immer groesser werdende Liste an datasets
        r.add_did({
            'scope': scope,
            'name': name + '.' + ts,
            'type': 'DATASET',
            ...
            success: function(data) {
                r.attach_dids({
                    ...
                    success: function(data2) {
                        console.log(data2);
                        //TODO: diese message vieleicht in move_did success function?
                        //alert(dids.length + ' files in this dataset have to be moved to WebDAV enabled RSEs first, please come back later.');
...
$(document).ready(function(){
    ...
    if ((storage.isSet('expiration') && Date.now() - storage.get('expiration') > 3600000) || !storage.isSet('expiration')) {
        ...
        var date = new Date(storage.get('expiration'));
