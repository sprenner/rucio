                            ...
                            if (state == 'AVAILABLE') {
                                ...
                                try{
                                    if (browser_enabled_rses.indexOf(rse) != -1 && !blacklisted){ 
                                        ...
                                        var options = {dids:scope + ':' + name, account:r.account, copies:1, lifetime:1, asynchronous:true, notify:true, rse_expression:get_rse_index(scope + ':' + name, browser_enabled_rses.length)};
                                        //console.log(browser_enabled_rses.length);
                                        console.log(options);
                                        str_rses += rse + "<a href=\"#\" onclick=\"moveFile();return false;\" style=\"color: rgb(0,255,0)\"><i class=\"fi-download size-18\"></i></a>";
			                        }
		                        } catch (err) {
