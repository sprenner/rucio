                            ...
                            if (state == 'AVAILABLE') {
                                ...
                                try{
                                    if (browser_enabled_rses.indexOf(rse) != -1 && !blacklisted){ 
                                        ...
                                        str_rses += rse + " <a href=\"#\" onclick=\"move_did('" + replica['scope'] + "','" + replica['name'] + "',false);return false;\" style=\"color: rgb(0,255,0)\"><i class=\"fi-download size-18\"></i></a>";
			                        }
		                        } catch (err) {
...
move_did = function(scope, name, is_dataset) {
...
get_metalink = function(scope, name) {
    ...
    if(true || reference_length <= 500){
        ...
        r.list_replicas({
            ...
            //TODO: dont do this if already replicating!!! Store list of rule IDs and query them on future calls? list_replication_rules_state or list_replication_rules
            success: function(ret_meta) {
                //var moving = false;
                //var notify = false;
                var meta_files = [];
                //console.log(ret_meta);
                /*$(ret_meta).find('file').each(function() {
                    var self_ref = this;
               
                    $(this).find('url').each(function() {
                        if($(this).text().indexOf('davs') == -1) {
                            moving = true;
                        } else {
                            moving = false;
                    ...
                    if(moving){
                        var identity = $(self_ref).find('identity').text();
                   
                  
                        notify = true;
                        meta_files.push(identity);
                    }
                });*/
                $(ret_meta).find('file').each(function() {
                ...
                r.list_replicas({
                    ...
                    success: function(ret) {
                        ...
                        if(move_amount == 0) {
                            ...
                            create_and_move_dataset('user.' + r.account, name + '.' + storage.get('move_timer'), files_to_move);                               
                        }
                        $('#meta_button').html('<a href="#" onclick="get_metalink(\'' + scope + '\',\'' + name + '\');return false;"><i title="Click to download metalink for this dataset." class="fi-download size-24"></i></a>');   
                    },
                    error: function(jqXHR, textStatus, errorThrown) {
...
create_and_move_dataset = function(scope, name, dids) {
    ...
    r.add_did({
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            if (JSON.parse(jqXHR.responseText)['ExceptionClass'] == 'DataIdentifierAlreadyExists') {
                ...
                r.did_get_rules({
                    ...
                    success: function(data) {
                        console.log(data);
                        if(data.length == 0) {
                           // Dataset exists, so just create the rules again
                           alert(dids.length + ' files in this dataset have to be moved to WebDAV enabled RSEs first, please come back later.');
                           move_did(scope, name, true);
                        } else {
...
$(document).ready(function(){
    ...
    if ((storage.isSet('move_timer') && Date.now() - storage.get('move_timer') > 172800000) || !storage.isSet('move_timer')) {
        storage.set('move_timer', Date.now());
        console.log('move_timer set: ' + storage.get('move_timer'));
    } else {console.log('move_timer found with value: ' + storage.get('move_timer'));}
});
