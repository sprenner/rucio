...
move_did = function(scope, name, is_dataset, no_of_files, attempt_no) {
    ...
    var options = {
        ...
        success: function(info) {
            ...
            if(is_dataset) {
                alert(no_of_files + ' file(s) in this dataset have to be moved to WebDAV enabled RSEs first, please come back later. You will be notified via email when all files have been moved.');
            }
        }, 
        error: function(jqXHR, textStatus, errorThrown) {
            console.log(jqXHR);
            ...
            success: function(ret_meta) {
                ...
                r.list_replicas({
                    ...
                    success: function(ret) {
                        ...
                        console.log(files_to_move.length);
    ...
    if(storage.isSet('moving_datasets') && found_dataset_list.length > 0) {     
        ...
        r.add_did({
            ...
            success: function(data) {
                ...
                //TODO: TESTTESTTEST, back to dids
               // var temp_dids = [];
               // temp_dids.push(dids[1]);
                r.attach_dids({
                    'scope': scope,
                    'name': name + '.' + ts,
                    'dids': dids, //TODO: TESTTEST
                    //'async': false,
                    success: function(data2) {
                        console.log(data2);                      
                        move_did(scope, name + '.' + ts, true, dids.length, 0); //TODO: TESTTEST
                    },
                    error: function(jqXHR, textStatus, errorThrown) {
                        console.log(jqXHR);
