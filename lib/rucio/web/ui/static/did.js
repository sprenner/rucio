...
load_replicas = function(scope, name) {
    ...
    console.log(available_dataset_rses);
    ...
    r.list_replicas({
        ...
        success: function(replicas) {
            ...
            //create object which contains all dav rse links which can be queried by replica name
            temp = {};
	    ...
	    $.each(replicas, function(index, replica) {
                ...
                $.each(sorted_rses, function(index, rse) {
                    ...
                    if (state == 'AVAILABLE') {
			            ...
			            available_dataset_rses[rse]['rses'].push({name: temp[replica['name']][rse][0].split("/").slice(-1)[0] , link: temp[replica['name']][rse][0].replace("davs", "https"), size: replica['bytes']});
                                }
				var link = temp[replica['name']][rse][0].replace("davs", "https");
				str_rses += rse;
				if(link.includes(".log")){		
                                   str_rses += "<a href=\"#\" onclick=\"runPyScript('" + temp[replica['name']][rse][0].replace("davs", "https")  +"');return false;\"><i title=\"Click to view this file from " + rse  + ".\" class=\"fi-eye size-18\"></i></a> "; 
				}
		...
		if(available_dataset_rses.hasOwnProperty(rse_id)){
		    dt.cell( rowIdx, colIdx).data(rse_id + ' <a href="data:application/octet-stream,' + encodeURIComponent(create_metalink(rse_id)) + '" target="_blank"  download="metalink_' + rse_id  + '.meta4"><i title="Click to download metalink for this dataset from ' + rse_id  + '." class="fi-download size-18"></i></a>').draw(false);
		}
	    });
	    ...
	    //THIS WORKS
	    /*var table = $('#dt_replicas').DataTable();
            $('#dt_replicas tbody').on('click', 'tr', function () {
                var data = table.row( this ).data();
                console.log( '2: You clicked on '+data.name+'\'s row' );
		console.log(data);
		//console.log(runPyScript('INPUT'));
            } );
            table.draw();*/
...
create_metalink = function(rse) {
    var links = available_dataset_rses[rse]['rses'];
    var xmlDoc = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" +
    "<metalink xmlns=\"urn:ietf:params:xml:ns:metalink\">";
    for(i = 0; i < links.length; i++){
        xmlDoc += "<file name=\"" + links[i]['name']  + "\">";
	xmlDoc += "<size>" + links[i]['size'] + "</size>";
	xmlDoc += "<url priority=\"1\">" + links[i]['link'] + "</url></file>";
    }
    xmlDoc += "</metalink>";
    return xmlDoc;
};

function extractLogfile(link, filename) {
    console.log("calling extract with: " + link + " " + filename)
    var tempHtml = $('#infomodal').html();    
    //$('#infomodal').html('<div id="loader_logpreview" "class="row"><div class="small-1 small-centered columns"><img src="/media/spinner.gif"></div>');
    var jqXHR = $.ajax({
        type: "GET",
        url: "/ui/extract",
        async: false,
	    ...
	    //$('#infomodal').html(tempHtml);
	    //res = res.replace(/\n/g, '<br />');
	    //bd = JSON.stringify(res).slice(1, -1);	 
	    var w = window.open();
	    //bd.replace(/(?:\r\n|\r|\n)/g, '<br />')
	    var str = '';
...
function runPyScript(input){
	...
	success: function(res) {
		...
		data.push(tmp);
	        //modalContent += "<a href=\"#\" onclick=\"extractLogfile('" + input  + "', '" + key + "');return false;\">" + key  + "</a>" + value + "</br>"
            });
	    ...
	    // modalContent += '</p><a class="close-reveal-modal" aria-label="Close">&#215;</a>';
    ...
    //$('#infomodal').html('<p>changed</p><a class="close-reveal-modal" aria-label="Close">&#215;</a>');
    ...
    r.list_dataset_replicas({
	    ...
	    $.each(replicas, function(index, replica) {
	     ...
	     r.list_replicas({
                'scope': scope,
                'name': name,
                success: function(f_replicas) {
                       console.log(f_replicas);
                    r.list_replicas({
                            'scope': scope,
                            'name': name,
                            'schemes': ['davs'],
			    ...
			    $.each(f_replicas, function(index, f_replica) {
				    ...
				    $.each(sorted_rses, function(index, rse) {
					    ...
					    if(state == 'AVAILABLE'){
						    if (browser_enabled_rses.indexOf(rse) != -1){
							    if(available_dataset_rses.hasOwnProperty(rse)){
								    available_dataset_rses[rse]['rses'].push({name: temp[f_replica['name']][rse][0].split("/").slice(-1)[0] , link: temp[f_replica['name']][rse][0].replace("davs", "https"), size: f_replica['bytes']});
							    }
						    }
					    }
				    });
