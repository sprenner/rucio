...
html_dataset_replicas_base = '<div id="t_dataset_replicas" class="columns panel">' +
    '<h4>Dataset Replicas <a href="#" data-reveal-id="infomodalmeta"><i title="Info" class="step fi-info size-24"></i></a> <span id="meta_button">tests</span></h4>' +  
                            ...
                            if (state == 'AVAILABLE') {
                                ...
                                try{
                                    if (browser_enabled_rses.indexOf(rse) != -1 && !blacklisted){ 
                                        ...
                                        str_rses += rse + " <a href=\"#\" onclick=\"moveFile('" + replica['scope'] + "','" + replica['name'] + "');return false;\" style=\"color: rgb(0,255,0)\"><i class=\"fi-download size-18\"></i></a>";
			                        }
		                        } catch (err) {
...
get_metalink = function(scope, name) {
    ...
    r.list_replicas({
        ...
        //TODO: execute only on button click, not on page load!! => new method
        //TODO: dont do this if already replicating!!!
        //TODO: create new Dataset of files to be moved.
        success: function(ret_meta) {
            var moving = false;
            var notify = false;
            $(ret_meta).find('file').each(function() {
                var self_ref = this;
                console.log('File: ' + $(self_ref).find('identity').text() + ' ____________________');
                $(this).find('url').each(function() {
                    if($(this).text().indexOf('davs') == -1) {
                        moving = true;
                    } else {
                        moving = false;
                        console.log('Found dav link, not moving file');
                        return false;
                    }
                });
                if(moving){
                    var identity = $(self_ref).find('identity').text();
                    console.log('Moving file: ' + identity);
                    //moveFile(identity.split(':')[0], identity.split(':')[1]);
                    notify = true;
            ...
            if(!notify) {
                ...
                alert('Some files in this dataset have to be moved to WebDAV enabled RSEs first, please come back later.');
                console.log(ret_meta);
                var wnd = window.open();
                wnd.document.write('<textarea>' + ret_meta + '</textarea>');
            }
        }});    
    } else {alert('Dataset contains more than 500 files, no metalink created.');}
};

move_dataset = function(scope, name) {
    moveFile(scope, name);
};
