...
load_replicas = function(scope, name) {
    ...
    r.list_replicas({
		...
		if(available_dataset_rses.hasOwnProperty(rse_id)){
		    dt.cell( rowIdx, colIdx).data(rse_id + ' <a href="data:application/octet-stream,' + encodeURIComponent(create_metalink(rse_id)) + '" target="_blank"  download="metalink_' + rse_id  + '.meta4">download</a>').draw(false);
		}
	    });
